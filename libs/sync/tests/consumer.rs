...
async fn test_consumer() {
    ...
    // Spawn the consumer
    let consumer_handle =
        Consumer::spawn(rate_limit, rx, shutdown_tx.clone(), move |message: Message| {
    ...
    assert_eq!(*invocations as usize, processed_items.len(), "Processed items count mismatch");
    let expected_messages_processed: Vec<&Message> =
        messages_to_send.iter().take(processed_items.len()).collect();
