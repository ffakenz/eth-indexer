...
mod tests {
    ...
    async fn test_moonwalk_polling_transfer_event_logs() -> Result<()> {
        ...
        // Start the engine
        let args = engine::args::MoonwalkArgs {
            address: *contract.address(),
            event: "Transfer(address,address,uint256)".to_string(),
            from_block: latest_block_hash,
        };
        let engine = Engine::start_moonwalk_logs(args, &node_client).await?;
