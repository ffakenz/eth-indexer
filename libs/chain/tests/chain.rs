...
mod tests {
    ...
    async fn test_polling_transfer_event_logs() -> Result<()> {
        ...
        let receipt = pending_mint_tx.get_receipt().await?;
        println!("Mint receipt: {receipt:?}");
        ...
        // Poll for transfer event logs
        let mut logs_stream = node_client
            .watch_logs(
                contract.address(),
