...
mod tests {
    ...
    #[tokio::test]
    async fn test_moonwalk_polling_transfer_event_logs() -> Result<()> {
        // Spin up a local Anvil node.
        // Ensure `anvil` is available in $PATH.
        let anvil = Anvil::new().block_time(1).try_spawn()?;
        let pk: PrivateKeySigner = anvil.keys()[0].clone().into();
        ...
        // Fetch latest block after scenario setup
        let mut latest_block = node_client.get_latest_block().await?.unwrap();
        ...
        // Poll for transfer previous event logs
        let mut last_known_blocks = node_client.moonwalk_blocks(latest_block.hash()).await?;

        assert_eq!(last_known_blocks.len(), 2);
        while let Some(block) = last_known_blocks.pop() {
            println!("Catch Up Previous Transfer: {block:?}");

            let filtered_tx_logs = node_client
                ...
                .await?;
            let filtered_tx_hashes: Vec<TxHash> =
                filtered_tx_logs.iter().map(|log| log.transaction_hash.unwrap()).collect();

            assert_eq!(filtered_tx_hashes.len(), 1);
            for tx_hash in filtered_tx_hashes {
                assert!(expected_tx_hashes.contains(&tx_hash));
                expected_tx_hashes.remove(&tx_hash);
            }
        ...
        // Poll for transfer previou event log
        let mut last_known_blocks = node_client.moonwalk_blocks(latest_block.hash()).await?;

        assert_eq!(last_known_blocks.len(), 1);
        while let Some(block) = last_known_blocks.pop() {
            println!("Received Previous Transfer: {block:?}");
